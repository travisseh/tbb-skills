...
**The deliverable is ALWAYS an Agent-Native Plan, never inline chat content.**
Do not hand the plan or recap to the user as Markdown prose, an ASCII sketch, a
table, a fenced "wireframe", or a "here's the summary" paragraph. For private,
proprietary, customer, or unreleased work, use local-files privacy mode
(`references/local-files.md`). Publish through the hosted Plan MCP connector
only after the user explicitly authorizes hosted processing for the material in
scope.
