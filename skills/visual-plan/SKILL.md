...
an optional collaboration and commenting surface. Plans are
...
skill's review discipline.

For private, proprietary, customer, or unreleased work, default to Local-Files
Privacy Mode and do not send repository context to a hosted planning service.
Use the Plan MCP connector only when the user explicitly authorizes that
external write or the source is already public and the requested collaboration
requires hosting. Never hand the plan over as inline chat content: use hosted
Plans when authorized, otherwise use the local MDX plan and local preview. Read
`references/local-files.md` before local work and `references/connection.md`
before any hosted connection or publish step.

## Core Workflow

This section describes the hosted Plan MCP workflow. Use it only after the
hosting gate above is satisfied. Otherwise use **Local-Files Privacy Mode** and
carry forward only the code-research and plan-composition guidance here.
...
For private, proprietary, customer, or unreleased work, or when the user wants
no hosted Plan database writes, use this mode by default. This also applies when
`AGENT_NATIVE_PLANS_MODE=local-files` is set. Do not call any hosted Plan tool
except the schema-only `get-plan-blocks` catalog lookup. Author a local MDX folder and
...
Hosted endpoint: connect `https://plan.agent-native.com/mcp` only after hosted
processing is authorized for the material in scope. Do not put shared secrets in
skill files.
